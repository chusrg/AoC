...
class TreeNode:
    ...
    def find_path(self, node_name: str, is_fft: bool, is_dac: bool, count: int) -> int:
        if node_name == self.name:
            return 1 if is_fft and is_dac else 0
        else:
            if self.name == "fft":
                is_fft = True
            if self.name == "dac":
        ...
        for child in self.childs:
            n += child.find_path(node_name, is_fft, is_dac, count)
...
with (open(input_data, encoding="utf-8") as f):
    ...
    print(node.find_path("out", False, False,0, []))